...
bool SfxPlayer::Init() {
  ...
  // Reminder chime. The fallback stays: a clip that fails the format gate at
  // runtime must still make a sound, because a reminder that makes NO sound is
  // a reminder that does not exist, and that is the one failure this feature
  // cannot afford. It should no longer fire — IDR_SFX_DUE ships in Runner.rc.
