...
param(
  [string]$Exe = (Join-Path $PSScriptRoot '..\..\Releases\slate_v1.0.2\slate.exe'),
  [string[]]$Scenario
)
...
function Scenario-S4 {
  ...
  $t = Stop-Sampler $smp
  $flash = @($t | Where-Object { $_.Zoomed -and $_.AboveNotepad -and ($_.Alpha -gt 0) })
...
function Scenario-S5 {
  ...
  $flash = @($t | Where-Object { $_.Visible -and (-not $_.Cloaked) -and (-not $_.Iconic) -and ($_.Alpha -gt 0) -and (Test-SampleRectNear $_ $R0) })
...
function Scenario-S7 {
  ...
  Start-Sleep -Milliseconds 300
  $t = Stop-Sampler $smp
  $flash = @($t | Where-Object { $_.Visible -and (-not $_.Cloaked) -and ($_.Alpha -gt 0) -and (Test-SampleRectNear $_ $R0) })
