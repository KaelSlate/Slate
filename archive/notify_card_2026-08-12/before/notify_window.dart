...
class _NotifySceneState extends State<_NotifyScene>
  ...
  /// Live horizontal offset while the card is being flicked away.
  double _dragX = 0;
  ...
  List<int>? _hitRect() {
    ...
    final right = (_dragging || _dragX > 0)
        ? media.size.width * dpr
        : (o.dx + box.size.width + _kPadSide) * dpr;
    return [
      ((o.dx - _kPadSide) * dpr).floor(),
  ...
  void _pushRegionAfterFrame() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _lastRegion = r;
      _channel.invokeMethod('region', r);
    });
  }
  ...
  Widget _travelling() {
    return GestureDetector(
      ...
      // Dragging toward the edge it came from is the natural direction; a drag
      // back into the screen is resisted, so the card never leaves its corner.
      onHorizontalDragStart: (_) {
        _dragging = true;
        _life?.cancel();
        // Open the window's shape to the right BEFORE the card starts moving.
        _pushRegionAfterFrame();
      },
      onHorizontalDragUpdate: (d) {
        setState(() {
          _dragX = math.max(0, _dragX + d.delta.dx);
        });
      },
      onHorizontalDragEnd: (d) {
        ...
        if (v > 320 || _dragX > AppTheme.notifyWidth * 0.33) {
          _flickOut(v);
        } else {
          _settleBack();
        }
      ...
      child: AnimatedBuilder(
        animation: Listenable.merge([_mw, _mh, _shadow, _exit, _collapse]),
        builder: (context, child) {
          ...
          // rather than an object stuck to the cursor.
          final dragFade =
              (1.0 - (_dragX / (AppTheme.notifyWidth * 0.9))).clamp(0.0, 1.0);
  ...
  Future<void> _flickOut(double velocity) async {
    ...
    _life?.cancel();
    final start = _dragX;
    final target = AppTheme.notifyWidth + AppTheme.notifySlideOvershoot;
    final sim = FrictionSimulation.through(
      start,
      target,
      math.max(velocity, 900),
      0,
    );
    final ticker = Stopwatch()..start();
    while (mounted && _dragX < target) {
      await SchedulerBinding.instance.endOfFrame;
      final t = ticker.elapsedMilliseconds / 1000.0;
      final x = sim.x(t);
      if (!mounted) return;
      setState(() => _dragX = x.isFinite ? x : target);
      if (t > 0.6) break;
    }
    await _close();
  }

  /// Not far enough: spring back home.
  void _settleBack() {
    ...
    void step() {
      ...
      final t = ticker.elapsedMilliseconds / 1000.0;
      final x = sim.x(t);
      setState(() => _dragX = x.isFinite && x > 0.5 ? x : 0);
      if (_dragX > 0) {
