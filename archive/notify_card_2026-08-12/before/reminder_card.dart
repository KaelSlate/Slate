...
class _ReminderCardShellState extends State<ReminderCardShell>
  ...
  void initState() {
    ...
    _merged = Listenable.merge([_hover, _light]);
  }
    ...
    return MouseRegion(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          final hover = Curves.easeOutCubic.transform(_hover.value);
          return Transform(
...
/// Behind the contents: the body, and the pool of light the pointer drags
/// across the glass.
class _ShellPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final base = acrylic
        ? AppTheme.background.withValues(alpha: 0.58)
    ...
    final bodyColor = Color.lerp(
      base,
      Color.alphaBlend(
          _green.withValues(alpha: AppTheme.notifyCollapseTint), base),
    ...
    // Faint, because this one passes UNDER live text and Liquid Glass's one
    // real sin is legibility. The strong light lives on the rim, where no glyph
    // is.
    //
    // THREE stops, not two. A two-stop radial is a linear alpha ramp — the
    // flattest falloff there is — and at 86 px out it still holds half its
    // peak. Real specular falls off nearer inverse-square, and without that the
    // "highlight" measured as a uniform +25 % lift across the whole card: a
    // wash, which is the one thing this card promises never to paint.
    //
    // And SQUASHED, like the rim. A circular pool of light on a card six times
    // wider than it is tall reads as exactly what it is — a disc stuck on the
    // surface, which is the "cheap circle under the cursor" everyone sees
    // immediately. Compressed vertically it becomes a band of light lying along
    // the glass, which is what light on a long flat surface actually looks like.
    if (light != null && hover > 0.001 && collapseT <= 0) {
      final a = AppTheme.notifySpecularAlpha * hover;
      final l = light!;
      final squash = Matrix4.identity()
        ..translateByDouble(l.dx, l.dy, 0, 1)
        ..scaleByDouble(1.0, 1.0 / AppTheme.notifyLensSquashY, 1.0, 1)
        ..translateByDouble(-l.dx, -l.dy, 0, 1);
      canvas.save();
      ...
      canvas.drawRect(
        ...
        Paint()
          ..shader = ui.Gradient.radial(
            l,
            AppTheme.notifySpecularRadius,
            [
              Colors.white.withValues(alpha: a),
              Colors.white.withValues(alpha: a * 0.30),
...
/// Over the contents: the glass edge. An even hairline all round, overhead
/// light on the top curve, light leaving the bottom edge (that is thickness) —
/// and the lensed highlight that concentrates wherever the pointer is.
class _ShellRimPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRSuperellipse(
      ...
      stroke()
        ..color = Color.lerp(
          Colors.white.withValues(alpha: AppTheme.glassRimEvenOpacity),
        ...
        ..shader = ui.Gradient.linear(
          ...
          [
            Colors.white.withValues(
                alpha: (_lerp(AppTheme.glassRimTopOpacity, 0.66, hover) * fade)
                    .clamp(0.0, 1.0)),
            Colors.white.withValues(alpha: 0.0),
          ],
          const [0.0, 0.35],
    ...
    // THE LENS. Not a glow on the surface — a concentration of light along the
    // edge, strongest at the pointer and gone within a hand's width. This is
    // the one stroke that makes the material read as glass rather than as a
    // dark rounded rectangle.
    //
    // SQUASHED VERTICALLY, and that is not a refinement — it is the difference
    // between rim light and a lamp under the table. The card is 55 px tall, so
    // a pointer at mid-height is equidistant from both rims; an isotropic light
    // hands them identical alpha and the overhead ratio measured 2.03 : 1 at
    // rest against 1.44 : 1 while hovering. The light source moved BELOW the
    // card. Compressing the gradient's vertical reach keeps the near edge lit,
    // the far edge dark, and the sun where the rest of this painter puts it.
    if (light != null && hover > 0.001 && collapseT <= 0) {
      final l = light!;
      final squash = Matrix4.identity()
        ..translateByDouble(l.dx, l.dy, 0, 1)
      ...
      canvas.drawRSuperellipse(
        ...
        stroke()
          ..shader = ui.Gradient.radial(
            l,
            AppTheme.notifyLensRadius,
            [
              Colors.white.withValues(alpha: AppTheme.notifyLensAlpha * hover),
              Colors.white.withValues(alpha: 0.0),
            ],
            const [0.0, 1.0],
            TileMode.clamp,
