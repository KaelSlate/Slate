...
class AppTheme {
  ...
  // NOTE: there was a `notifyAccentWidth` here, documented as "the only place
  // priority shows on a card". Nothing read it and no stripe was ever drawn —
  // a specification for a solution to a problem the card still has. Priority
  // currently shows ONLY in the colour of the time, which is the smallest text
  // on the card and invisible to a colour-blind reader. Either draw the stripe
  // or admit the gap; do not leave a constant standing in for a decision.
  ...
  /// The specular that follows the pointer. Liquid Glass's actual behaviour on
  /// 26: light CONCENTRATES where you are ("lensing"), it does not scatter.
  ///
  /// The first pass did scatter, and measurably. Half the card's diagonal is
  /// 174 px, so a 170 px radius put EVERY pixel of the card inside the light;
  /// with a two-stop ramp — the flattest falloff that exists — the dimmest
  /// column still measured 25 % brighter than at rest. That is not a highlight,
  /// it is a vignette running backwards, and it is exactly the grey wash this
  /// card claims twice over never to use. 96 px is 1.7 card-heights: past that
  /// the body is left alone, so what travels is a POOL.
  static const double notifySpecularAlpha = 0.075;
  static const double notifySpecularRadius = 96.0;

  /// The rim is where light may be strong: no glyph lives there.
  static const double notifyLensAlpha = 0.50;
  static const double notifyLensRadius = 120.0;

  /// ...and it must be SQUASHED, or it destroys the thing it decorates.
  ///
  /// The card is 55 px tall, so a pointer at mid-height is equidistant from the
  /// top and bottom rims and an isotropic light hands them identical alpha.
  /// Measured, hovering dropped the overhead ratio from 2.03 : 1 to 1.44 : 1 —
  /// the light source flipped to somewhere below the card, erasing the forty
  /// lines of rim work that put it above. Squashing the gradient's vertical
  /// reach by this factor keeps the near edge bright, the far edge dark, and
  /// the sun where it belongs.
  static const double notifyLensSquashY = 3.2;

  /// Per-frame easing of the light toward the pointer. At 0.16 it lags by
  /// roughly seven frames — the light CHASES the cursor instead of being glued
  ...
  static const Duration notifyPressDuration = Duration(milliseconds: 150);
