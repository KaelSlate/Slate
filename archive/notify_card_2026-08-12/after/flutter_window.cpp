...
bool FlutterWindow::OnCreate() {
  ...
  notify_probe_msg_ = ::RegisterWindowMessageW(L"Slate.NotifyProbe");
  shell_channel_ =
  ...
  shell_channel_->SetMethodCallHandler(
      [this](const auto& call, auto result) {
        ...
        if (method == "showPill") {
          ...
          // The reply is load-bearing: false means the card did NOT go up, and
          // the scheduler must leave the moment unspoken so it can try again.
          bool shown = false;
          if (notify_sink_ && call.arguments()) {
            shown = notify_sink_(*call.arguments());
          }
          result->Success(flutter::EncodableValue(shown));
        } else if (method == "notifyHide") {
          if (notify_hide_cb_) notify_hide_cb_();
          result->Success();
        } else {
...
void FlutterWindow::SendNotifyAction(const flutter::EncodableValue& value) {
  if (shell_channel_) {
    shell_channel_->InvokeMethod(
        "notifyAction",
        std::make_unique<flutter::EncodableValue>(value));
...
FlutterWindow::MessageHandler(HWND hwnd, UINT const message,
  ...
  // Harness: raise a card through the real path, with fixed harmless content.
  if (message == notify_probe_msg_ && notify_probe_msg_ != 0) {
    if (notify_sink_) {
      const int rows = wparam < 1 ? 1 : (wparam > 3 ? 3 : static_cast<int>(wparam));
      static const char* kTitles[3] = {"Dinner with Anna", "Call the landlord",
                                       "Ship the draft"};
      static const char* kTimes[3] = {"18:00", "18:00", "18:30"};
      flutter::EncodableList cards;
      for (int i = 0; i < rows; ++i) {
        cards.push_back(flutter::EncodableValue(flutter::EncodableMap{
            {flutter::EncodableValue("id"),
             flutter::EncodableValue(std::string("probe") + std::to_string(i))},
            {flutter::EncodableValue("title"),
             flutter::EncodableValue(std::string(kTitles[i]))},
            {flutter::EncodableValue("time"),
             flutter::EncodableValue(std::string(kTimes[i]))},
            {flutter::EncodableValue("dayMs"), flutter::EncodableValue(0)},
        }));
      }
      notify_sink_(flutter::EncodableValue(flutter::EncodableMap{
          {flutter::EncodableValue("cards"), flutter::EncodableValue(cards)}}));
      // The chime, because this path bypasses the only place that fires it.
      // ReminderScheduler calls chime() after present() returns, and the probe
      // goes straight to the sink — so the harness was silent by construction,
      // which made it useless for judging the one thing you cannot see.
      // 0.60 mirrors Sfx._dueGain; a harness that plays at a different level
      // is a harness that lies about the level.
      SfxPlayer::Instance().Play("due", 0.60f);
    }
    return 0;
  }
